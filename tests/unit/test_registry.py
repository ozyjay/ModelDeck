...
def test_packaged_runtime_registry_is_versioned(tmp_path) -> None:
    ...
    assert registrations["scenechat-qwen35"].package.version == "0.9.1"
...
def test_gpt_oss_cpu_moe_is_a_distinct_runtime_template(tmp_path) -> None:
    gguf = tmp_path / "gpt-oss-120b-MXFP4.gguf"
    gguf.write_bytes(b"gguf")
    profile = create_local_profile(
        LocalProfileRequest(
            model_id="ggml-org/gpt-oss-120b-GGUF",
            revision="a" * 40,
            alias="gpt-oss-cpu-moe",
            artifact_id="gpt-oss-120b-mxfp4",
            runtime_template_id="gpt-oss-llama-vulkan-cpu-moe",
            context_length=8192,
            maximum_new_tokens=256,
        ),
        cache_root=tmp_path,
        artifact_path=gguf,
        port=8630,
        configuration_support="gpt-oss-llama-vulkan-cpu-moe",
    )

    assert profile.runtime_template_id == "gpt-oss-llama-vulkan-cpu-moe"
    assert profile.runtime_template_version == "0.9.1"
    assert profile.preferred_runtime == "llama-vulkan"
    assert profile.settings["execution_preset"] == "vulkan-cpu-moe"
...
def test_approved_qwen35_candidate_profile_binds_server_derived_manifest_identity(tmp_path) -> None:
    gguf = tmp_path / "Qwen_Qwen3.5-9B-Q8_0.gguf"
    gguf.write_bytes(b"gguf")

    profile = create_local_profile(
        LocalProfileRequest(
            model_id="bartowski/Qwen_Qwen3.5-9B-GGUF",
            revision="a" * 40,
            alias="qwen35-9b-adaptive",
            artifact_id="qwen35-9b-q8-123456789abc",
            runtime_template_id="qwen35-local-q8-vulkan-adaptive",
            context_length=8192,
        ),
        cache_root=tmp_path,
        artifact_path=gguf,
        candidate_manifest_id="qwen35-9b-q8-123456789abc",
        port=8630,
        configuration_support="qwen35-local-q8-vulkan-adaptive",
    )

    assert profile.dtype == "q8_0"
    assert profile.settings["runtime_profile"] == "qwen35-approved-q8-vulkan"
    assert profile.settings["candidate_manifest_id"] == "qwen35-9b-q8-123456789abc"
    assert profile.settings["thinking_mode"] == "adaptive"
