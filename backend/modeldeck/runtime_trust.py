...
TRUSTED_RUNTIME_IMPLEMENTATIONS = {
    ...
    for implementation in (
        ...
        TrustedRuntimeImplementation(
            ...
            generation_family=GenerationFamily.VISION_LANGUAGE,
            capabilities=frozenset(
                {"chat", "streaming", "cancellation", "image_input", "tool_calling", "reasoning"}
            ),
            template_settings=frozenset(
                {
                    "context_length",
        ...
        TrustedRuntimeImplementation(
            id="qwen38-fp8-vision-language-transformers-rocm",
            display_name="Qwen3.8 native FP8 vision-language Transformers ROCm",
            generation_family=GenerationFamily.VISION_LANGUAGE,
            capabilities=frozenset({"chat", "streaming", "cancellation", "image_input", "structured_output"}),
            template_settings=frozenset(
                {
        ...
        TrustedRuntimeImplementation(
            id="qwen38-fp8-chat-transformers-rocm",
            display_name="Qwen3.8 native FP8 text chat Transformers ROCm",
            generation_family=GenerationFamily.AUTOREGRESSIVE,
