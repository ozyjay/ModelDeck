...
try {
    ...
    if ($ManagementAvailable) {
        try {
            ...
            throw 'ModelDeck management is running but the gateway is unavailable. Run scripts/operations/stop.ps1, then retry.'
        }
    }
    else {
        Write-Host 'ModelDeck is not running; starting local services for the benchmark.'
        $StartedServices = $true
        & (Join-Path $PSScriptRoot '../operations/run.ps1')
    ...
    $Arguments = @(
        './scripts/benchmarks/benchmark_models.py',
    ...
    if ($StartedServices) {
        & (Join-Path $PSScriptRoot '../operations/stop.ps1')
    }
}
