...
$Route = $Live.capabilities | Where-Object { $_.public_name -eq $RouteName }
if (-not $Route) { throw "The active Routing Profile has no capability named '$RouteName'." }
if (@($Route).Count -ne 1) { throw "The public capability name '$RouteName' is ambiguous." }
