...
if (-not (Test-Path '.venv-qwen3-tts-rocm72/bin/python')) {
    throw 'Run pwsh -NoProfile -File scripts/setup/setup_qwen3_tts_rocm72.ps1 first.'
}
& .venv-qwen3-tts-rocm72/bin/python scripts/qualification/qualify_qwen3_tts.py `
    --endpoint $Endpoint `
    --model $Model `
