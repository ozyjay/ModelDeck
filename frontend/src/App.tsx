...
export default function App() {
  return <NotificationProvider><OperatorConsole /></NotificationProvider>;
}
...
function OperatorConsole() {
  ...
  const setError = useNotification(true, "Action failed");
  ...
  const operate = async (worker: Worker, operation: WorkerOperation) => {
    ...
    try {
      ...
      setError(operation === "start" && reason instanceof ApiError && reason.status === 429
        ? `Start blocked (HTTP 429) — ${message}` : message);
    }
    ...
    <div className="app-shell">
      ...
      <main className="main-content">
        ...
        {!health || !hardware || !telemetry || !thermal || !gateway ? <Unavailable retry={refresh} />
          ...
          : view === "workers" ? <WorkersView workers={workers} models={models} templates={templates} thermal={thermal} pending={pending} operate={operate} refresh={refresh} openDay={health.configuration_locked} />
...
function SetupView({ models, workers, templates, live, refresh, openDay }: {
  ...
  const setFeedback = useNotification(true, "Setup needs attention");
  ...
  const review = async () => {
    ...
    catch (reason) { setFeedback(messageFrom(reason), true); }
  ...
  const publish = async () => {
    ...
    try {
      ...
      setFeedback(messageFrom(reason), true);
    }
    finally { setBusy(false); }
  };
  ...
  return <div className="view-stack">
    ...
    {setup ? <section className="panel setup-progress" aria-live="polite"><PanelHeading title="Create and test" detail={humanise(setup.state)} />
      ...
      {setup.state === "failed" && <div className="button-row"><button disabled={!setup.error?.retryable} onClick={() => void postJson<CapabilitySetup>(`/api/capability-setups/${setup.id}/retry`).then(setSetup).catch((reason) => setFeedback(messageFrom(reason), true))}>Try again</button><button className="secondary" onClick={reset}>Adjust configuration</button></div>}
...
function LiveView({ live, workers, models, thermal, operate, pending, refresh }: {
  ...
  const setRouteFeedback = useNotification();
...
function RoutingProfilesView({ profiles, workers, contracts, openDay, refresh }: {
  ...
  const setFeedback = useNotification();
  ...
  useEffect(() => {
    ...
    const timer = window.setTimeout(() => {
      putJson(`/api/routing-profiles/${draft.id}/draft`, draft)
        ...
        .catch((reason) => { setSaveState("Save failed"); setFeedback(messageFrom(reason), true); });
    }, 500);
  ...
  const toggleRouting = async () => {
    ...
    if (selected.active) {
      await deleteJson(`/api/routing-profiles/${draft.id}/active`);
      setFeedback("Routing Profile disabled. No Workers were stopped.");
    } else if (selected.latest_revision !== null) {
      await postJson(`/api/routing-profiles/${draft.id}/revisions/${selected.latest_revision}/publish`);
      setFeedback(`Routing Profile enabled at revision ${selected.latest_revision}. No Workers were started.`);
    }
    await refresh();
  };
  ...
  const deleteProfileReason = openDay
    ? "Configuration is locked, so Routing Profiles cannot be deleted."
    : selected?.latest_revision
      ? "Published Routing Profiles are retained as immutable revision history and cannot be deleted. Disable routing to remove it from the live gateway."
      : undefined;

  return <div className="view-stack">
    <div className="view-actions"><p>Routing Profiles publish reusable local capabilities for any application or event.</p><button disabled={openDay} onClick={() => void createProfile().catch((reason) => setFeedback(messageFrom(reason), true))}>Create Routing Profile</button></div>
    {!selected || !draft ? <section className="empty-state"><h2>No Routing Profiles yet</h2><p>Create a profile after configuring at least one Worker.</p></section> : <div className="event-layout">
      <aside className="panel event-list">{profiles.map((profile) => <button className={`event-select ${profile.definition.id === draft.id ? "active" : ""}`} key={profile.definition.id} onClick={() => setSelectedId(profile.definition.id)}><span><strong>{profile.definition.name}</strong><small>{profile.active ? `Live revision ${profile.active_revision}` : profile.latest_revision ? `Published revision ${profile.latest_revision}` : "Draft only"}</small></span><StateBadge state={profile.active ? "ready" : "stopped"} /></button>)}</aside>
      <CollapsiblePanel sectionId={`profile-${draft.id}`} title={draft.name} detail={`${selected.active ? `Live revision ${selected.active_revision}` : selected.latest_revision ? `Published revision ${selected.latest_revision}` : "Draft"} · ${saveState}`} className="event-detail" accessory={<StateBadge state={selected.active ? "ready" : "stopped"} />}>

        <div className="button-row event-actions"><button className="secondary" disabled={Boolean(duplicateNames.size)} onClick={() => void validate().catch((reason) => setFeedback(messageFrom(reason), true))}>Validate</button><button disabled={openDay || saveState === "Saving…" || Boolean(duplicateNames.size)} onClick={() => void publish().catch((reason) => setFeedback(messageFrom(reason), true))}>Publish routing</button>{selected.active ? <button className="secondary" disabled={openDay} onClick={() => void toggleRouting().catch((reason) => setFeedback(messageFrom(reason), true))}>Disable routing</button> : <button className="secondary" disabled={openDay || selected.latest_revision === null} onClick={() => void toggleRouting().catch((reason) => setFeedback(messageFrom(reason), true))}>Enable routing</button>}<button className="secondary" disabled={openDay || !selected.latest_revision} onClick={() => void deleteJson(`/api/routing-profiles/${draft.id}/draft`).then(refresh)}>Discard draft</button><button className="secondary" onClick={() => void getJson<{ revisions: RoutingProfileRevision[] }>(`/api/routing-profiles/${draft.id}/revisions`).then((result) => setRevisions(result.revisions))}>History</button><button className="secondary danger" disabled={Boolean(deleteProfileReason)} title={deleteProfileReason} onClick={() => void deleteProfile().catch((reason) => setFeedback(messageFrom(reason), true))}>Delete Routing Profile</button></div>
        {deleteProfileReason && <p className="manifest-note">{deleteProfileReason}</p>}
...
function runtimeParameterDefaults(template?: RuntimeTemplate, worker?: Worker): WorkerParameterValues {
  const templateContextLength = integerSetting(template?.settings, "context_length", 2048);
  return {
    ...
    contextLength: template?.fixed_context_length === true
      ? templateContextLength
      : integerSetting(worker?.settings, "context_length", templateContextLength),
...
function contextLengthLimit(template: RuntimeTemplate, maximumContextLength?: number | null) {
  if (template.fixed_context_length === true) return integerSetting(template.settings, "context_length", 0);
  return Math.min(32768, maximumContextLength ?? 32768);
}

function parametersAreValid(template: RuntimeTemplate, values: WorkerParameterValues, maximumContextLength?: number | null) {
  const maximumContext = contextLengthLimit(template, maximumContextLength);
  const validContext = !["autoregressive", "vision-language"].includes(template.generation_family)
    || (template.fixed_context_length === true
      ? values.contextLength === integerSetting(template.settings, "context_length", 0)
      : values.contextLength >= 256 && values.contextLength <= maximumContext);
...
function WorkerParameterFields({ template, values, onChange, prefixCacheAvailable = false, capabilityId, maximumContextLength }: {
  template: RuntimeTemplate;
  values: WorkerParameterValues;
  onChange: (values: WorkerParameterValues) => void;
  prefixCacheAvailable?: boolean;
  capabilityId?: string | null;
  maximumContextLength?: number | null;
}) {
  ...
  return <>
    <div className="runtime-fields worker-parameter-fields">
      ...
      {hasContext && <label>Context length<small>{template.fixed_context_length === true ? "Fixed by the trusted runtime" : `256–${maximumContext.toLocaleString()} tokens`}</small><input aria-label="Context length" type="number" min={template.fixed_context_length === true ? values.contextLength : 256} max={maximumContext} value={values.contextLength} disabled={template.fixed_context_length === true} onChange={(event) => update({ contextLength: event.target.valueAsNumber })} /></label>}
...
function WorkersView({ workers, models, templates, thermal, pending, operate, refresh, openDay }: { workers: Worker[]; models: ModelEntry[]; templates: RuntimeTemplate[]; thermal: ThermalStatus; pending: ReadonlySet<string>; operate: (worker: Worker, operation: WorkerOperation) => Promise<void>; refresh: () => Promise<void>; openDay: boolean }) {
  ...
  return <div className="view-stack"><div className="view-actions worker-view-heading"><p>A Worker is one configured, startable service. Its name is editable; its execution identity is not. Use Replace to change safe model limits without mutating the original Worker.</p></div>
    ...
    {!workers.length ? <section className="empty-state"><h2>No Workers configured</h2><p>Create one from the Models view. ModelDeck does not create packaged Worker cards.</p></section> : !sorted.length ? <section className="empty-state compact"><h2>No Workers match these filters</h2><p>Try a different name, model, capability, state or runtime.</p><button className="secondary" onClick={clearFilters}>Clear filters</button></section> : <div className="worker-grid">{sorted.map((worker) => {
      ...
      const visualBudgetActive = workerModel ? workerHasAllowedImageCapability(workerModel, worker) : false;
...
function workerHasAllowedImageCapability(model: ModelEntry, worker: Worker): boolean {
  return model.potential_capabilities.some((capability) =>
    capability.effective_allowed
    && capability.traits.includes("image-input")
    && capability.available_runtime_template_ids.includes(worker.runtime_template_id ?? "")
  );
}
...
function ModelWorkerSummary({ model, workers, openDay, removingWorker, onRemove }: { model: ModelEntry; workers: Worker[]; openDay: boolean; removingWorker: string | null; onRemove: (worker: Worker) => Promise<void> }) {
  ...
  return <section className={`model-worker-summary${configured.length ? " has-workers" : ""}`} aria-label={`Workers for ${model.model_id}`}>
    ...
    <div hidden={collapsed}>{configured.length ? <div className="model-worker-list">{configured.map((worker) => <div className="model-worker-item" key={worker.id}>
      <div><strong>{worker.name}</strong><small>{humanise(worker.runtime)} · {humanise(worker.lifecycle)}</small></div>
