    ...
    {!selected || !draft ? <section className="empty-state"><h2>No Events yet</h2><p>Create an Event after configuring at least one Worker.</p></section> : <div className="event-layout">
      ...
      <CollapsiblePanel sectionId={`event-${draft.id}`} title={draft.name} detail={`${selected.active ? `Live revision ${selected.active_revision}` : "Draft"} · ${saveState}`} className="event-detail" accessory={<StateBadge state={selected.active ? "ready" : "stopped"} />}>
        ...
        <div className="event-editor">
          ...
          <div className="editor-section-heading"><div><h3>Demos</h3><p className="muted">Each Demo contains references to the shared Routes it uses.</p></div><button disabled={openDay} onClick={() => setDraft({ ...draft, demos: [...draft.demos, { id: crypto.randomUUID(), name: "New Demo", route_ids: [] }] })}>Add Demo</button></div>
          <div className="demo-editor-list">{draft.demos.map((demo) => <article className="route-editor demo-editor" aria-label={`Demo ${demo.name}`} key={demo.id}><div className="route-editor-title"><label>Demo name<input value={demo.name} disabled={openDay} onChange={(event) => setDraft({ ...draft, demos: draft.demos.map((item) => item.id === demo.id ? { ...item, name: event.target.value } : item) })} /></label><button className="secondary danger" disabled={openDay} onClick={() => setDraft({ ...draft, demos: draft.demos.filter((item) => item.id !== demo.id) })}>Remove</button></div><div className="demo-route-section"><h4>Routes used by this Demo</h4>{draft.routes.length ? <div className="demo-route-reference-list">{draft.routes.map((route) => { const included = demo.route_ids.includes(route.id); return <label className={`demo-route-reference${included ? " selected" : ""}`} key={route.id}><input type="checkbox" checked={included} disabled={openDay} onChange={(event) => toggleDemoRoute(demo.id, route.id, event.target.checked)} /><span><strong>{route.display_name}</strong><code>{route.public_name}</code></span></label>; })}</div> : <p className="muted">Create a shared Route below, then include it in this Demo.</p>}</div></article>)}</div>
