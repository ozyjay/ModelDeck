...
import type {
  CompatibilityTest, GatewayStatus, HardwareProbe, LiveCapability, LiveState, ManagementHealth, ModelEntry,
    ...
    <div className="app-shell">
      ...
      <main className="main-content">
        ...
        {!health || !hardware || !telemetry || !thermal || !gateway ? <Unavailable retry={refresh} />
          ...
          : view === "workers" ? <WorkersView workers={workers} templates={templates} pending={pending} operate={operate} refresh={refresh} openDay={health.configuration_locked} />
  ...
  if (!workers.length || !live.active_profiles.length) return (
    <div className="view-stack">
      ...
      <CollapsiblePanel sectionId="live-setup" title="Setup checklist" detail={`${models.length} cached Models discovered`}>
        <ol className="setup-list"><li className={models.length ? "done" : ""}>Discover a cached Model</li><li className={workers.length ? "done" : ""}>Create a Worker</li><li className={live.active_profiles.length ? "done" : ""}>Create and publish a Routing Profile</li><li>Start and smoke-test the capability’s Worker</li></ol>
      </CollapsiblePanel>
    </div>
  );
...
const WAYFINDER_PREFIX_CACHE_MODELS = new Set([
...
function WorkerParameterFields({ template, values, onChange, prefixCacheAvailable = false }: {
  ...
  return <>
    <div className="runtime-fields worker-parameter-fields">
      ...
      {prefixCacheAvailable && <label>WayFinder prefix cache<small>Enable only after physical qualification</small><select aria-label="WayFinder prefix cache" value={values.prefixCacheEnabled ? "enabled" : "disabled"} onChange={(event) => update({ prefixCacheEnabled: event.target.value === "enabled" })}><option value="disabled">Disabled</option><option value="enabled">Enabled</option></select></label>}
    </div>
