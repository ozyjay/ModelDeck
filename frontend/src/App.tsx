  ...
  return <div className="view-stack">
    ...
    {!selected || !draft ? <section className="empty-state"><h2>No Routing Profiles yet</h2><p>Create a profile after configuring at least one Worker.</p></section> : <div className="event-layout">
      ...
      <CollapsiblePanel sectionId={`profile-${draft.id}`} title={draft.name} detail={`${selected.active ? `Live revision ${selected.active_revision}` : selected.latest_revision ? `Published revision ${selected.latest_revision}` : "Draft"} · ${saveState}`} className="event-detail" accessory={<StateBadge state={selected.active ? "ready" : "stopped"} />}>
        ...
        <div className="button-row event-actions"><button className="secondary" disabled={Boolean(duplicateNames.size)} onClick={() => void validate().catch((reason) => setFeedback(messageFrom(reason)))}>Validate</button><button disabled={openDay || saveState === "Saving…" || Boolean(duplicateNames.size)} onClick={() => void publish().catch((reason) => setFeedback(messageFrom(reason)))}>Publish routing</button>{selected.active ? <button className="secondary" disabled={openDay} onClick={() => void toggleRouting().catch((reason) => setFeedback(messageFrom(reason)))}>Disable routing</button> : <button className="secondary" disabled={openDay || selected.latest_revision === null} onClick={() => void toggleRouting().catch((reason) => setFeedback(messageFrom(reason)))}>Enable routing</button>}<button className="secondary" disabled={openDay || !selected.latest_revision} onClick={() => void deleteJson(`/api/routing-profiles/${draft.id}/draft`).then(refresh)}>Discard draft</button><button className="secondary" onClick={() => void getJson<{ revisions: RoutingProfileRevision[] }>(`/api/routing-profiles/${draft.id}/revisions`).then((result) => setRevisions(result.revisions))}>History</button><button className="secondary danger" disabled={Boolean(deleteProfileReason)} title={deleteProfileReason} onClick={() => void deleteProfile().catch((reason) => setFeedback(messageFrom(reason)))}>Delete Routing Profile</button></div>
