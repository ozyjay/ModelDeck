...
function DemoSetEditor({ draft, setDraft, deployments, adapters, pending, save, cancel }: {
  ...
  return <form className="demo-set-editor" onSubmit={(event) => { event.preventDefault(); save(); }}>
    ...
    <div className="route-editor-list">{draft.routes.map((route, routeIndex) => <article className="route-editor" key={route.id}>
      ...
      <div className="provider-editor-list">{route.providers.map((binding, providerIndex) => <div key={`${binding.deployment_id}-${providerIndex}`}><select aria-label={`Provider ${providerIndex + 1} for ${route.id}`} value={binding.deployment_id} onChange={(event) => updateRoute(routeIndex, { providers: route.providers.map((item, index) => index === providerIndex ? { ...item, deployment_id: event.target.value } : item) })}>{deployments.map((deployment) => <option value={deployment.id} key={deployment.id}>{deployment.id} · {deployment.model.model_id}</option>)}</select><label className="provider-priority">Priority<input aria-label={`Priority for ${binding.deployment_id}`} type="number" min="0" max="10000" value={binding.priority} onChange={(event) => updateRoute(routeIndex, { providers: route.providers.map((item, index) => index === providerIndex ? { ...item, priority: Number(event.target.value) } : item) })} /></label><button type="button" className="secondary danger" onClick={() => updateRoute(routeIndex, { providers: route.providers.filter((_, index) => index !== providerIndex) })}>Remove</button></div>)}</div>
    </article>)}</div>
