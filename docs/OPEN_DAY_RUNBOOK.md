...
1. Confirm `.venv` and `.venv-rocm72` already exist; never install or update dependencies
   at the event.
2. Run `pwsh -NoProfile -File scripts/operations/check_ports.ps1` and
   `pwsh -NoProfile -File scripts/operations/check_environment.ps1`.
3. Run `pwsh -NoProfile -File scripts/operations/run.ps1 -LockConfiguration`. ModelDeck is always
...
For a booth session, `pwsh -NoProfile -File scripts/booth/run_booth.ps1` combines steps 3 and 4
...
ModelDeck through a background watcher; `scripts/operations/stop.ps1` can be used instead. Neither
...
2. Run `pwsh -NoProfile -File scripts/verification/verify_scenechat_snapshot.ps1`. Retain its immutable
...
4. Run `pwsh -NoProfile -File scripts/smoke/smoke_rocm_scenechat.ps1`. It preflights port 8000 and
...
- Use `pwsh -NoProfile -File scripts/operations/stop.ps1` to stop workers, gateway, and management
...
`pwsh -NoProfile -File scripts/q4/burn_in_diffusiongemma_selected_preset.ps1` only when the
