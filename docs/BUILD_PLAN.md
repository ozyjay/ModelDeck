...
4. **Text diffusion — implemented and hardware-gated:** pinned local DiffusionGemma load
   ...
   memory-recovery gate and its fixed-workload two-hour burn-in; the BF16 worker remains
   the explicit compatibility and quality baseline. Real management-preset transitions
   and operator rehearsal remain separate Open Day gates rather than implementation
   blockers.
