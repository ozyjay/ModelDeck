...
Builds are intentionally offline. Populate `packaging/fedora/wheelhouse/` with every wheel
needed by the pinned control, ROCm, and Q4 requirements, then create and review
`packaging/fedora/wheelhouse.sha256`. The manifest has one lower-case SHA-256 and filename per
wheel. It is not generated by the build command because review of downloaded runtime artefacts
is an explicit release step.

```powershell
pwsh -NoProfile -File scripts/packaging/build_fedora_rpm.ps1 `
  -Wheelhouse /secure/release-wheelhouse `
  -WheelhouseManifest /secure/release-wheelhouse.sha256 `
  -OutputDirectory dist/fedora
```

The command fails if a wheel is unlisted, missing, or has a different digest. It uses `pip
--no-index` and rejects the live ROCm URLs in the source requirements after replacing them with
their pinned package versions. No model download or package-manager installation occurs during
the build.
